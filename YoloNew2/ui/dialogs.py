...
class AugmentationSettingsDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.settings = {
            ...
            "target_size": None # New setting for image resizing
        }
    ...
    def _create_ui(self):
        ...
        self._create_preprocessing_tab() # Add new tab
    ...
    def _load_settings(self):
        ...
        # Preprocessing
        target_size_value = self.settings.get("target_size", None)
        if target_size_value is None:
            self.target_size_combo.setCurrentIndex(0) # "None"
        else:
            index = self.target_size_combo.findData(target_size_value)
            if index != -1:
                self.target_size_combo.setCurrentIndex(index)
            else:
                self.target_size_combo.setCurrentIndex(0) # Default to "None" if not found
    ...
    def _save_settings(self):
        ...
        # Preprocessing
        selected_data = self.target_size_combo.currentData()
        self.settings["target_size"] = selected_data # This will be None or an int
    ...
    def _create_preprocessing_tab(self):
        """Create the preprocessing tab for image resizing."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        
        group_box = QGroupBox("Image Resizing (Applied Before Augmentations)")
        form_layout = QFormLayout(group_box)

        self.target_size_combo = QComboBox()
        self.target_size_combo.addItem("None (Original Size)", None)
        self.target_size_combo.addItem("416x416", 416)
        self.target_size_combo.addItem("640x640", 640)
        self.target_size_combo.addItem("832x832", 832)
        self.target_size_combo.addItem("1280x1280", 1280)
        
        form_layout.addRow("Target YOLO Input Size:", self.target_size_combo)
