...
class AugmentationConfig:
    ...
    def __init__(self):
        ...
        self.target_size: Optional[int] = None # For resizing to a square, e.g., 640 for 640x640
...
class ImageAugmenter:
    ...
    def create_transform_pipeline(self, include_geometric=None, include_color=None, 
        ...
        # Apply resizing and padding first if target_size is specified
        if config.target_size and isinstance(config.target_size, int) and config.target_size > 0:
            transforms_list.extend([
                A.LongestMaxSize(max_size=config.target_size, p=1.0),
                A.PadIfNeeded(
                    min_height=config.target_size, 
                    min_width=config.target_size, 
                    border_mode=cv2.BORDER_CONSTANT, 
                    value=[0, 0, 0],  # Black padding
                    p=1.0
                )
            ])
            print(f"DEBUG: Added resize/pad to target size: {config.target_size}x{config.target_size}")
